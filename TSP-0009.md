...
```
OpCheckTemplateVerify  (OpReserved - 0x50)
```
...
* **Non-upgraded nodes:** In Tapscript context, treat as `OpSuccessx` → soft-fork safe.
* **Implementation Note:** Tondi uses `OpReserved` (0x50) which acts as `OpSuccessx` in Tapscript context, providing soft-fork safety while maintaining Taproot compatibility.  
