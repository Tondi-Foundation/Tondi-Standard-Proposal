...
**SHOULD**: Define minimal PSBT extension fields (nonce commitments, partial nonces, agg nonce, partial sig, cohort bitmap proposal, etc.) and specify **wallet interoperability** minimum set.
...
- Tapscript-only; new OPs introduced via OP_SUCCESSx encoding; old nodes unconditionally pass, new nodes tighten to require CISA validation.
...
- **Key-Path CISA**: Discuss in future new witness version (isolated from existing rules, maintaining soft fork).  
...
- Lock OP_SUCCESSx encoding and version bit;  
- Release cross-implementation test vectors (Rust/Go/C++/Python);  
- Testnet ≥ 3 months, including adversarial aggregation/reorg/rate-limiting experiments;  
...
pub struct CisaEnvelope {
    pub magic: [u8; 4],           // "CISA"
    pub version: u8,              // 0x01
    pub flags: u16,              // bit0: SIGHASH_ALL only
...
- **CISA Envelope Storage**: Tondi uses `signature_script` field instead of separate witness structure; CISA envelope should be stored as transaction-level extension
...
4. **Testing Framework**: Adapt test vectors to Tondi's transaction format and testing infrastructure
5. **OpCode System**: Extend Tondi's opcode system to support CISA-specific opcodes via OP_SUCCESSx encoding
...
**Appendix A: Size Estimation Example (Non-Normative)**  
