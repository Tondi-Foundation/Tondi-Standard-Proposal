...
The tapscript signature opcode rules are modified by defining new opcodes encoded as OP_SUCCESSx (e.g., opcode values in the 0xba-0xfe range reserved for future soft forks). Specific OP_SUCCESSx encoding values will be specified in the **deployment parameter table** (e.g., OP_SUCCESS200/OP_SUCCESS201/OP_SUCCESS202 corresponding to OP_CHECKSIG_TSP/OP_CHECKSIGVERIFY_TSP/OP_CHECKSIGADD_TSP respectively). Implementations **MUST** use these fixed numbers to avoid implementation divergence. Unupgraded nodes MUST treat these as success (no further execution), while upgraded nodes MUST perform the following:
...
- New opcodes: `OP_CHECKSIG_TSP`, `OP_CHECKSIGVERIFY_TSP`, `OP_CHECKSIGADD_TSP`
...
opcode_list! {
    opcode OP_CHECKSIG_TSP<0xba, 0>($self, $vm) {
        $vm.check_signature_tsp()
    }
    
    opcode OP_CHECKSIGVERIFY_TSP<0xbb, 0>($self, $vm) {
        $vm.check_signature_verify_tsp()
    }
    
    opcode OP_CHECKSIGADD_TSP<0xbc, 0>($self, $vm) {
        $vm.check_signature_add_tsp()
    }
}
```
...
1. **Add new opcodes** - OP_CHECKSIG_TSP, OP_CHECKSIGVERIFY_TSP, OP_CHECKSIGADD_TSP
